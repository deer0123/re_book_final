...
### 7.4. 김성철
>#### 7.4.1. 최종
>- 최종 프로젝트는 중간 프로젝트를 기반으로 JSP를 React로 전환하였고, AWS와 도커를 활용한 배포와 Github Actions를 통한 자동화를 구현하였습니다.
>- 리액트 페이지를 만드는 과정에서 새로운 문법을 익히고 원하는 화면이 나올 때까지 수정하는 과정을 거쳤습니다.
>- 이메일 인증 과정을 백엔드에서 프런트엔드로 로직을 일부 옮겨 왔습니다.
>- 백엔드와 프런트엔드를 작성하는 것과 달리 AWS와 도커 그리고 깃허브액션은 설정을 하는 가정에서 실수가 잦았다.
